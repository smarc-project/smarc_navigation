...
void MBESReceptor::init(){
    ...
    try{
        tf_listener.waitForTransform(base_frame_, sss_l_frame_, ros::Time(0), ros::Duration(10));
...
void MBESReceptor::mbesReadingsCB(const sensor_msgs::LaserScanConstPtr &mbes_l_msg,
    ...
    if(sss_right_.landmarks_.size() > 0){
        std::vector<double> aux_vec(sss_right_.landmarks_.back());
        // Create marker for RVIZ
        geometry_msgs::PointStamped landmark_msg;   // Move to PointCloud and expand for more than one landmark detected
        landmark_msg.header.stamp = mbes_r_msg->header.stamp;
        landmark_msg.header.frame_id = "lolo_auv/sonarright_link";
        landmark_msg.point.x = aux_vec.at(0);
        landmark_msg.point.y = aux_vec.at(1);;
        landmark_msg.point.z = aux_vec.at(2);;
