...
void EKFCore::batchDataAssociation(std::vector<Eigen::Vector3d> z_t, const utils::MeasSensor &sens_type){
    ...
    for(unsigned int i = 0; i<z_t.size(); i++){
        ...
        temp_sigma.block(0,0,6,6) = Sigma_hat_temp.block(0,0,6,6);
        ...
        for (unsigned int j=0; j<temp_num_lm; j++){
            if(munkres_matrix(j,i) == 0){
                ...
                if(j >= lm_num_){ // If new landmark added
                    ...
                    utils::addLMtoFilter(mu_hat_, Sigma_hat_, corresp_list.at(j*(i+1)).landmark_pos_, new_lm_cov);
                }
                ROS_INFO_STREAM("Sequential update");
                sequentialUpdate(corresp_list.at(j*(i+1)), temp_sigma);
            }
        }
    }
...
void EKFCore::batchUpdate(std::vector<CorrespondenceClass> const& vec_cij, Eigen::MatrixXd temp_sigma){
...
void EKFCore::sequentialUpdate(CorrespondenceClass const& c_i_j, Eigen::MatrixXd temp_sigma){
    ...
    std::cout << "LM chosen: " << std::endl;
    std::cout << c_i_j.i_j_.second << std::endl;
...
std::tuple<Eigen::VectorXd, Eigen::MatrixXd> EKFCore::ekfUpdate(){
    ...
    std::cout << "Final mu: " << std::endl;
    std::cout << mu_ << std::endl;

    std::cout << "Final sigma: " << std::endl;
    std::cout << Sigma_ << std::endl;

    lm_num_ = (mu_.rows() - 6) / 3;
    std::cout << "Final number of landmarks: " << std::endl;
