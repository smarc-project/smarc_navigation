...
from sbg_driver.msg import SbgEkfQuat, SbgEkfEuler
import math
...
class VehicleDR(object):
    ...
    def __init__(self):
        ...
        self.pub_odom_stim = rospy.Publisher("/sam/dr/stim_odom", Odometry, queue_size=100)
        ...
        try:
            ...
            try:
                ...
                if self.init_heading:
                    ...
                    self.tfMapOdom.transform.translation.z = self.init_depth
    ...
    def dr_timer(self, event):
        ...
        if self.init_heading:
            ...
            odom_msg.twist.twist.linear.x = vel_t[0]
            odom_msg.twist.twist.linear.y = vel_t[1]
            odom_msg.twist.twist.linear.z = vel_t[2]
            odom_msg.twist.twist.angular.x = vel_t[3]
            odom_msg.twist.twist.angular.y = vel_t[4]
            odom_msg.twist.twist.angular.z = vel_t[5]
    ...
    def stim_cb(self, imu_msg):
        ...
        if self.init_stim and self.init_depth is not None:

            dt = imu_msg.header.stamp.to_sec() - self.t_stim_prev
            self.t_stim_prev = imu_msg.header.stamp.to_sec()
            
            rot_t_stim = np.array([imu_msg.orientation.x,
                                    imu_msg.orientation.y,
                                    imu_msg.orientation.z,
                                    imu_msg.orientation.w])
            
            euler_t = tf.transformations.euler_from_quaternion(rot_t_stim)
            ...
            self.rot_t_stim = np.array(self.rot_t_stim) + self.vel_rot * dt
            # Measure roll and pitch directly
            self.rot_t_stim[0] = euler_t[0]
            self.rot_t_stim[1] = euler_t[1]
            # Wrap yaw
            self.rot_t_stim[2] = (self.rot_t_stim[2] + np.pi) % (2 * np.pi) - np.pi
            
            # DVL data coming in
            rot_mat_t = self.fullRotation(self.rot_t_stim[0], self.rot_t_stim[1], self.rot_t_stim[2])
            if self.dvl_on:
                # Integrate linear velocities from DVL
                # If last DVL msg isn't too old
                if self.t_now - self.t_dvl_prev < self.dvl_period and \
            ...
            step_t = np.matmul(rot_mat_t, self.lin_vel_t * dt)
            # step_t = np.array([0.0001,0.,0.])
            self.pos_t_stim[0:2] += step_t[0:2]

            # Measure depth directly
            self.pos_t_stim[2] = self.base_depth - self.init_depth
            ...
            # odom_msg.pose.covariance = [0.] * 36
            # odom_msg.pose.pose.orientation = Quaternion(*quat_t)
            # self.pub_odom_stim.publish(odom_msg)
