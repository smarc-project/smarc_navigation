...
class VehicleDR(object):
    def __init__(self):
        ...
        self.odom_top = rospy.get_param(
            "~odom_topic", "/sam/dr/dvl_dr"
        )  # topic used in the launch file for the DVL sensor
        ...
        self.pose_prev = [0.0] * 6
        self.init_heading = False
        self.init_m2o = False

        # Stim integration
        self.rot_t = [0.0] * 3
        self.t_stim_prev = 0.0
        ...
        self.dvl_on = False
        self.dvl_latest = DVL()
        self.dvl_latest.velocity.x = 0.0
        self.dvl_latest.velocity.y = 0.0
        self.dvl_latest.velocity.z = 0.0

        # Depth measurements: init to zero
        self.b2p_trans = [0.0] * 3
        self.depth_meas = False  # If no press sensor available, assume surface vehicle
        self.base_depth = 0.0  # abs depth of base frame

        # Motion model
        self.sam = SAM()
        self.mm_on = False
        self.mm_linear_vel = [0.0] * 3
        dr = np.clip(0.0, -7 * np.pi / 180, 7 * np.pi / 180)
        self.u = [0.0, dr]
        ...
        try:
            rospy.loginfo(
                "DR Waiting for transform %s to %s"
                % ("sam/base_link", self.press_frame)
            )
            self.b2d_tf = tfBuffer.lookup_transform(
                "sam/base_link",
                self.press_frame,
                rospy.Time(0),
            ...
            rospy.loginfo(
                "DR: got transform %s to %s" % ("sam/base_link", self.press_frame)
            )
            self.depth_meas = True

        except:
            rospy.logwarn(
                "DR node: could not get transform %s to %s"
        ...
        self.stim_sub = rospy.Subscriber(
            self.stim_topic, Imu, self.stim_cb, queue_size=10
        )
        self.depth_sub = rospy.Subscriber(
            self.depth_top, PoseWithCovarianceStamped, self.depth_cb
        )
        ...
        self.thrust_cmd_sub = rospy.Subscriber(
            self.thrust_topic, ThrusterAngles, self.thrust_cmd_cb
        )
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer(
            [self.thrust1_sub, self.thrust2_sub], 20, slop=20.0, allow_headerless=False
        )
        ...
        try:
            ...
            try:
                ...
                if self.init_heading:
                    rospy.loginfo(
                        "DR node: broadcasting transform %s to %s"
                    ...
                    quat = quaternion_from_euler(
                        euler[0], euler[1], euler[2] - np.pi / 2
                    )
                    # quat = [0., 0., 0., 1.]
                    ...
                    self.tfMapOdom.transform.translation.x = 0.0
                    self.tfMapOdom.transform.translation.y = 0.0
                    self.tfMapOdom.transform.translation.z = (
                        self.base_depth
                    )  # self.depth_base
    ...
    def uw_gps_cb(self, uw_gps_msg):
        ...
        self.tfUTMMap.transform.translation.z = 0.0
    ...
    def dr_timer(self, event):
        ...
        if self.init_stim:
            ...
            pose_t = np.concatenate(
                [self.pos_t, self.rot_t]
            )  # Catch latest estimate from IMU
            rot_vel_t = (
                self.vel_rot
            )  # TODO: rn this keeps the last vels even if the IMU dies
            ...
            if True:
                ...
                if (
                    self.t_now - self.t_dvl_prev < self.dvl_period
                    and abs(self.dvl_latest.velocity.y) < 0.2
                    and abs(self.dvl_latest.velocity.x) < 1.5
                    and self.dvl_latest.velocity.x > -1
                ):
                    # self.dvl_latest.velocity.x > -0.1:

                    lin_vel_t = np.array(
                        [
                            self.dvl_latest.velocity.x,
                ...
                # # Otherwise, integrate motion model estimate
                    ...
                    self.lin_acc_t = np.array(
                        [self.lin_acc_t[0], -self.lin_acc_t[1], 0.0]
                    )
            ...
            # 2D base link frame
            quat_euler_t = tf.transformations.quaternion_from_euler(0.0, 0.0, pose_t[5])
            self.br.sendTransform(
                [pose_t[0], pose_t[1], pose_t[2]],
                quat_euler_t,
                rospy.Time.now(),
    ...
    def thrust_cb(self, thrust1_msg, thrust2_msg):
        dr = np.clip(
            -self.thrust_cmd.thruster_horizontal_radians,
            -7 * np.pi / 180,
            7 * np.pi / 180,
        )
    ...
    def depth_cb(self, depth_msg):
        if self.depth_meas:
            self.base_depth = depth_msg.pose.pose.position.z - np.abs(
                self.b2d_tf.transform.translation.x
            ) * np.sin(self.rot_t[1])
